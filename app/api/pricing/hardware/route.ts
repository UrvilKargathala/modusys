...
export async function GET() {
  ...
  return NextResponse.json(items.map((i) => ({
    ...
    unitId: i.unitId, description: i.description, mrp: i.mrp, discountPct: i.discountPct,
...
export async function PUT(req: Request) {
  ...
  await replaceCollection(prisma.hardwarePriceItem, rows.map((r) => ({
    ...
    brandId: String(r.brandId), unitId: String(r.unitId), description: String(r.description ?? ""),
